...
assert_contains 'XBOX_LIVE_MTU="1380"'
assert_contains 'XBOX_LIVE_PORT="3074"'
assert_contains 'rdr pass on $VPN_IF inet proto udp from any to ($VPN_IF) port $XBOX_LIVE_PORT -> $XBOX_IP port $XBOX_LIVE_PORT'
assert_contains 'rdr pass on $vpn_if inet proto udp from any to ($vpn_if) port $XBOX_LIVE_PORT -> $XBOX_IP port $XBOX_LIVE_PORT'
assert_contains 'Xbox 360 Live/GTA IV'
